...
1. **Load artifacts** from the active feature:
   - Resolve branch: use `{{args}}` if provided (e.g. `004-ws4-composite-components-migration`), else `git branch --show-current`
   - Required: `spec.md`, `plan.md`, `tasks.md` — look in `specs/[branch]/` first, then `docs/[branch]/` if not found
   - Optional: constitution from `AGENTS.md`
