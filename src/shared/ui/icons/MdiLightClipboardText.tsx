...
/** MdiLightClipboardText — ported verbatim from design-system/components/icons/MdiLightClipboardText.jsx
 *  (figma node 17:23 mdi-light:clipboard-text). Paints with `currentColor`, so colour it from the
 *  parent rather than passing a fill (spec 002 FR-014). */
export function MdiLightClipboardText({ size = 24, className, ...rest }: SVGProps<SVGSVGElement> & { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
      ...
      {...rest}
    >
      <g transform="translate(3.0 2.0) scale(1.0 1.0)">
