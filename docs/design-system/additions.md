...
| ~~`RecordUnavailableScreen`~~ | ~~UNAVAILABLE~~ | — | **Removed 2026-09-26** (spec 008). It was the shared response for a request that did not exist or was not yours. With `/requests/:id` retired there is no record address, and every id gets `NotFoundScreen`, so FR-012a holds without it. |
