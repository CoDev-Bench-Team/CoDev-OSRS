...
| **My Requests table** | The `04 - My Requests` frame's five columns over the seeded source. Built as a stand-in so the panel had a *View details* to open from; BEN-44 (spec 009) made it the list, sorted newest first, to the frame's geometry (spec 009 D5). | The frame lists its sample rows out of date order; newest first is BEN-44's acceptance (spec 009 D2). **Flagged to the designer.** |
