...
- [x] T025 [US1] [BEN-67] Seed a `Cancelled` request (REQ-2026-1791) so `04.2 - Cancelled` opens from the list, and strike the Cancelled pill through as drawn — `src/features/requests/detail/seeded-employee-request-source.ts`, `src/app/seeded-request-ids.ts`, `src/shared/ui/data-display/StatusPill.tsx`, `scripts/check-request-detail.mjs`
- [x] T026 [BEN-71] Third review of #38: one `summarizeItems(names, shown)` and one `NO_VALUE` for My Requests and the Requests Queue (plan's dedupe after #36); correct the struck-through pill's reach in `additions.md` — `src/features/requests/format.ts`, `src/features/requests/queue/queue-model.ts`, `src/features/requests/history/MyRequestsPage.tsx`, `docs/design-system/additions.md`
