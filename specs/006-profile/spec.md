...
- **FR-010**: The production build of Profile MUST NOT fabricate, sample or seed assigned-equipment rows, and MUST NOT model individual units to produce them (D1, constitution VIII). A development-only stub whose rows are visibly synthetic MAY exist solely to exercise FR-007a and FR-007b; it MUST be absent from the production build. *(Amended 2026-09-23.)*
...
- **SC-002**: A search of the Profile feature, excluding its development-only stub, finds no literal name, email, office, asset tag or date for any real or seeded user. *(Amended 2026-09-23.)*
