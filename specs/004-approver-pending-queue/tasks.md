...
- [x] T020 [BEN-75] Review follow-ups: Pagination keeps focus at the ends (`aria-disabled`), leaves announcements to the page, and ignores the current page; `updateQuery` treats re-selecting the current value as no change, so the page stays (FR-022); one Search focus indicator with a token-derived inset; the sort value narrowed rather than cast; the projection memoised; and `check-queue.mjs`, the queue's own `verify` gate for FR-004–FR-008, FR-014 and FR-019–FR-023 — `src/shared/ui/data-display/Pagination.tsx`, `src/shared/ui/forms/Search.tsx`, `src/features/requests/queue/QueuePage.tsx`, `src/features/requests/queue/queue-model.ts`, `scripts/check-queue.mjs`, `scripts/verify.mjs`
...
- T020 arose from two review rounds after T019. Until it, FR-019–FR-023 were verified only by hand; the gate makes them part of `npm run verify`.
